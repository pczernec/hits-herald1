...
from . import Algorithm, VariableDiscountSACPolicy
...
class OffPolicyAlgorithm(Algorithm):
    ...
    supported_flat_offpol_algos = {
        "VariableDiscountSAC": VariableDiscountSACPolicy,
    ...
    class _Transition:
        def __init__(
            self, subtask_tr, env_info, child_feedback, algo_info, session_info=None
        ):
            self.subtask_tr = subtask_tr
            self.env_info = env_info
            self.child_feedback = child_feedback
            self.algo_info = algo_info
            self.sess_info = session_info
    ...
    def __init__(
        ...
        flat_algo_name,
        model,
        fully_random_fraction,
        flat_algo_kwargs=None,
        buffer_size=100000,
        batch_size=128,
        learning_starts=0,
        ...
        if flat_algo_name not in self.supported_flat_offpol_algos:
            raise ValueError(
                'Flat off policy RL algorithm "{}" not supported.'.format(
                    flat_algo_name
                )
            )
        ...
        if flat_algo_name in ("SAC", "VariableDiscountSAC") and "alpha" in self._flat_algo_kwargs:
            if isinstance(self._flat_algo_kwargs["alpha"], list) and not isinstance(
                self._flat_algo_kwargs["alpha"][0], torch.optim.Optimizer
            ):
                target_entropy = self._flat_algo_kwargs["alpha"][0]
                log_alpha = torch.tensor(
                    [self._flat_algo_kwargs["alpha"][1]],
                    dtype=torch.float,
                    requires_grad=True,
                )
                alpha_optim = torch.optim.Adam(
                    [log_alpha], lr=self._flat_algo_kwargs["alpha"][2]
                )
                self._flat_algo_kwargs["alpha"] = (
                    target_entropy,
                    log_alpha,
                    alpha_optim,
                )
        ...
        self._flat_algo_kwargs["action_range"] = [-1.0, 1.0]
    ...
    def get_policy(self, name, observation_space, action_space):
        ...
        nets = {
            ...
            f"critic{'1' if self._flat_algo_name in ('SAC', 'VariableDiscountSAC') else ''}": self._model.critics[
                0
            ],
            f"critic{'1' if self._flat_algo_name in ('SAC', 'VariableDiscountSAC') else ''}_optim": self._model.critic_optims[
                0
            ],
        }
        if self._flat_algo_name in ("SAC", "VariableDiscountSAC"):
        ...
        self._flat_algorithm = self._flat_algo_class(
            **nets, **self._flat_algo_kwargs, action_space=flat_gym_action_space
        )
    ...
    def add_experience(
        ...
        transition = self._Transition(
            ...
            algo_info=algo_info,
            session_info=deepcopy(sess_info)
        )
        self._episode_transitions.append(transition)
        self._actions_taken += 1

        # tensorboard logging
        self._log_to_tensorboard(parent_info, sess_info, subtask_trans)
    ...
    def _log_to_tensorboard(self, parent_info, sess_info, subtask_trans):
            ...
            if self._tb_writer is not None:
                ...
                if self._log_q_values and len(self._episode_transitions) == 1:
                    ...
                    with torch.no_grad():
                        if self.device == "cuda":
                            q_values = [
                                critic(obs[None], act[None]).cpu().numpy()
                                for critic in self._model.critics
                            ]
                        else:
                            q_values = [
                                critic(obs[None], act[None]).numpy()
                                for critic in self._model.critics
                            ]
                        q_value = min(q_values)
                    mode = "test" if sess_info.testing else "train"
                    self._tb_writer.add_scalar(
                        f"{self.name}/{mode}/q_value", q_value, sess_info.total_step
                    )
                if (
                        self._flat_algo_name in ("SAC", "VariableDiscountSAC")
                        and not sess_info.testing
                        and parent_info is None
                ):
                    with torch.no_grad():
                        net_out, _ = self._model.actor(obs[None])
                        _, sigma = net_out
                        if self.device == "cuda":
                            entropy = np.log(sigma.cpu().numpy()).sum() + (
                                    0.5 + np.log(np.sqrt(2.0 * np.pi))
                            ) * len(sigma)
                        else:
                            entropy = np.log(sigma.numpy()).sum() + (
                                    0.5 + np.log(np.sqrt(2.0 * np.pi))
                            ) * len(sigma)
                    self._tb_writer.add_scalar(
                        f"{self.name}/train/entropy", entropy, sess_info.total_step
                    )
    ...
    def learn(self, current_step):
        ...
        if (
            ...
            and current_step > self._learning_starts
        ):
            for _ in range(int(n_grad_steps)):
                batch, indices = self._replay_buffer.sample(self._batch_size)
                batch = self._flat_algorithm.process_fn(
                    batch, self._replay_buffer, indices
                )
                ...
                if self._tb_writer is not None:
                    for key, item in results.items():
                        self._tb_writer.add_scalar(
                            f"{self.name}/train/{key}", item, current_step
                        )
