...
from ..data import FlatTransition, SessInfo
...
class Graph:
    ...
    def __init__(self, name, entry_node, env_action_space):
        self.name = name
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}\nNodes: {[type(node).__name__ for node in self._nodes]}"
    ...
    def register_experience(self, env_info, sess_info):
        ...
        # active node is almost always (or rather always the LL node)
        (
            parent_interruption,
            interrupting_node,
            include_to_budget
        ) = self.active_node.check_parent_interruption(
        ...
        (
            self.active_node,
            self.active_parent,
            self.active_parent_info,
        ) = self.active_node.register_experience(
            env_info, Node.sink_identification_feedback, sess_info, interrupting_node, include_to_budget=include_to_budget
        )
    ...
    def load_state(self, state=None, dir_path=None):
        ...
        assert {state, dir_path} != {
            None,
            None,
        }, "Either state dict or directory with state files has to be specified."
    ...
    def log_additional_info_wandb(self, sess_info:SessInfo=None):
        pass

    def reset_episodes_info(self):
