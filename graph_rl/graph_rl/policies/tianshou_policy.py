...
class TianshouPolicy(Policy):
    ...
    def __call__(self, observation, algo_info, testing=False):
        ...
        with torch.no_grad():
            ...
            if (
                not deterministic
                and self._fully_random_fraction > 0.0
                and np.random.rand() < self._fully_random_fraction
            ):
                ...
                if deterministic:
                    ...
                    action_batch = self._ts_policy.forward(batch)
                    if self.device == 'cuda':
                        action = action_batch.act[0].detach().cpu().numpy()
                    else:
                        action = action_batch.act[0].detach().numpy()
        ...
        # TODO: why the child id is hard-codded
        return 0, action
