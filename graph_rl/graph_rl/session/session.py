...
from gym import GoalEnv, Wrapper
import numpy as np
from tianshou.data.utils.converter import to_numpy
import torch
from torch.utils.tensorboard import SummaryWriter

from scripts.run.loggers import get_interruption_logger
from graph_rl.data import EnvInfo, SessInfo
from graph_rl.logging import CSVLogger, read_csv_log
...
class Session:
    def __init__(self, graph, env):
        ...
        elif isinstance(env, Wrapper) and isinstance(env.env, GoalEnv):
            self._goal_based_env = True
        else:
    ...
    def step_env_and_graph(
        ...
        render_frequency,
        episode,
        total_step_multiplier=1.0,
    ):
        # get atomic action from forward pass through graph
        action = self.graph.get_atomic_action(
            env_info.new_obs, sess_info, testing=sess_info.testing
        )
        ...
        new_env_info = EnvInfo(
            ...
            info=info,
            time_step=self.env.time_step if hasattr(self.env, "time_step") else None,
        )
    ...
    def run(
        ...
        assert {type(n_steps), type(n_episodes)} == {
            int,
            type(None),
        ...
        if verbose >= 1:
            ...
            if isinstance(self.env, Wrapper):
                print("Env class: {}".format(type(self.env.env).__name__))
            else:
            ...
            for node_id, node in enumerate(self.graph._nodes):
                print("-----")
                print(
                    f"Node id: {node_id}\nNode: {node}\nNode alg: {type(node.algorithm).__name__}"
                )
        ...
        if csv_logdir is not None:
            ...
            logger_train = CSVLogger(
                os.path.join(csv_logdir_run, "session_train.csv"),
                fieldnames_train,
                append=append_to_logfiles,
            )
            logger_test = CSVLogger(
                os.path.join(csv_logdir_run, "session_test.csv"),
                fieldnames_test,
                append=append_to_logfiles,
            )
        else:
        ...
        env_info = EnvInfo(
            new_obs=self._process_env_observation(self.env.reset()), done=True
        )
        sess_info = SessInfo(
            ep_step=0, total_step=total_step_init, learn=learn, testing=False
        )
        self.graph.reset()

        t_start = time.process_time()

        interruption_logger = get_interruption_logger(self.env, self.graph)
        while (
            (n_steps is None or sess_info.total_step < n_steps)
            and (n_episodes is None or episode < n_episodes)
            and (max_runtime is None or time.process_time() - t_start < max_runtime)
            and checkpoint_succeeded
        ):
            ...
            if early_stopping:
                if early_stopping.is_check_time(sess_info.total_step):
                    tf_events_filename = [name for name in os.listdir(tb_logdir) if 'tfevents' in name][0]
                    path = os.path.join(tb_logdir, tf_events_filename)
                    checkpoint_succeeded = early_stopping.checkpoint(path)

            if train:
                env_info, graph_done = self.step_env_and_graph(
                    env_info, sess_info, render, render_frequency, episode
                )
            ...
            if not train or env_info.done or graph_done:
                ...
                if writer is not None:
                    writer.add_scalar(
                        "env/return/train", ep_return, sess_info.total_step
                    )
                    writer.add_scalar(
                        "env/ep_length/train", sess_info.ep_step, sess_info.total_step
                    )
                    if success_reward is not None:
                        writer.add_scalar(
                            "env/success/train", int(success), sess_info.total_step
                        )

                    interruption_logger.check_and_log_interruption(sess_info)
                    self.graph.log_additional_info_wandb(sess_info)
                    self.graph.reset_episodes_info()
                ...
                if test and episode % test_frequency == 0:
                    ...
                    for test_episode in range(test_episodes):
                        ...
                        while True:
                            env_info, graph_done = self.step_env_and_graph(
                                ...
                                sess_info_test,
                                test_render,
                                test_render_frequency,
                                test_episode,
                                1.0 / test_episodes,
                            )

                            if (
                                success_reward is not None
                                and env_info.reward == success_reward
                            ):
                                success_test = True
                            ...
                            if env_info.done or graph_done:
                                ...
                                if success_test:
                                    cumulative_env_return_test_success += (
                                        env_return_test
                                    )
                                cumulative_success_test += float(success_test)
                                break

                        interruption_logger.check_and_log_interruption(sess_info_test)
                        self.graph.log_additional_info_wandb(sess_info)
                        self.graph.reset_episodes_info()
                    ...
                    if cumulative_success_test > 0.0:
                        # Calculate time of average success episode
                        avg_return_test_success = (
                            cumulative_env_return_test_success / cumulative_success_test
                        )
                    else:
                    ...
                    if writer is not None:
                        ...
                        writer.add_scalar(
                            "env/return/test", avg_return_test, sess_info.total_step
                        )
                        writer.add_scalar(
                            "env/ep_length/test", avg_length_test, sess_info.total_step
                        )
                        if success_reward is not None:
                            writer.add_scalar(
                                "env/success/test",
                    ...
                    if logger_test is not None:
                        ...
                        if success_reward is not None:
                            ...
                            writer.add_scalar(
                                "env/success/test_proper_reward",
                                avg_return_test_success*avg_success_test - (1-avg_success_test)*self.env.env.max_episode_length
                                if avg_return_test_success is not np.nan
                                else -self.env.env.max_episode_length,
                                sess_info.total_step,
                            )
        ...
        for arg_name in self.run.__code__.co_varnames[: self.run.__code__.co_argcount]:
            if arg_name != "self" and type(locals()[arg_name]) in [
                str,
                bool,
                int,
                float,
        ...
        session_props["timed_out"] = (
            max_runtime is not None and t_elapsed >= max_runtime
        )
