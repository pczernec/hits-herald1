...
# TODO: Refactor and make separate class for Herald and HiTS
...
class EnvSPSubtaskSpec(ShortestPathSubtaskSpec):
    def __init__(self, max_n_actions, env, map_to_env_goal, 
                ...
                use_original_reward_of_env:bool=False,
                use_discounted_env_reward:bool=False,
                discounting_hl_gamma:float=0.99,
                replace_achieved_reward=False,
                reward_normalization_coefficient=None
                ):
        super().__init__(constant_failure_return)
        self._max_n_actions = max_n_actions
        ...
        self._use_original_reward_of_env = use_original_reward_of_env
        self._discounting_hl_gamma = discounting_hl_gamma
        self._use_discounted_env_reward = use_discounted_env_reward
        self._replace_achieved_reward = replace_achieved_reward
        self._reward_normalization_coefficient = reward_normalization_coefficient
        if use_original_reward_of_env:
            assert reward_normalization_coefficient is not None, \
            "When using environment reward you need to set reward_normalization_coefficient"
