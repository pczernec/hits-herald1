...
class TimedGoalSubtaskSpec(SubtaskSpec):
    ...
    def __init__(
        ...
        delta_t_max=None,
        delta_t_min=0.0,
        delta_t_const: int = None,  # actual number of steps, unscaled
        **kwargs
    ):
        super().__init__()
        ...
        if delta_t_ach is None and delta_t_const is None:
            ...
            space_dict["delta_t_ach"] = BoxSpace(low=[a_min], high=[1.0])
        self._parent_action_space = DictSpace(space_dict)

    @property
    def default_delta_t(self):
        return self._delta_t_const

    @property
    def include_delta_t_in_obs(self):
        return self._delta_t_const is None
    ...
    def unconvert_time(self, t):
        ...
        if not self.include_delta_t_in_obs:
            return t

        return (t + 1.0) / 2.0 * self._delta_t_max
    ...
    def convert_time(self, t):
        ...
        return t / self._delta_t_max * 2.0 - 1.0
    ...
    def get_desired_timed_goal(self, env_obs, parent_info):
        ...
        if self.include_delta_t_in_obs:
            tg = TimedGoal(
                goal=parent_info.action["goal"],
                delta_t_ach=parent_info.action["delta_t_ach"][0]
                if self._delta_t_ach is None
                else self._delta_t_ach,
                delta_t_comm=parent_info.action["delta_t_ach"][0]
                if self._delta_t_comm is None
                else self._delta_t_comm,
            )
        else:
            tg = TimedGoal(
                goal=parent_info.action["goal"],
                delta_t_ach=self.default_delta_t,
                delta_t_comm=self.default_delta_t
            )
        return tg
...
class BoxTGSubtaskSpec(TimedGoalSubtaskSpec):
    def __init__(
        ...
        super(BoxTGSubtaskSpec, self).__init__(
            self._space,
            self._space,
            delta_t_ach,
            delta_t_comm,
    ...
    def goal_achievement_criterion(self, achieved_goal, desired_goal, parent_info):
        ...
        for factor, threshold in zip(
            self.factorization, self._goal_achievement_threshold
        ):
            if (
                np.linalg.norm(achieved_goal[factor] - desired_goal[factor])
                >= threshold
            ):
                return False
        return True
...
class DictInfoHidingTGSubtaskSpecBase(TimedGoalSubtaskSpec):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._goal_keys_spec = goal_keys_spec
        ...
        super(DictInfoHidingTGSubtaskSpecBase, self).__init__(
            goal_space,
            partial_obs_space,
            delta_t_ach,
            delta_t_comm,
    ...
    def map_to_goal(self, partial_obs):
        return {
            key: value for key, value in partial_obs.items() if key in self._goal_keys
        }

    def map_to_partial_obs(self, env_obs, parent_info):
        return {
            key: value
            for key, value in get_obs_from_gym(env_obs).items()
            if key in self._partial_obs_keys
        }
...
class DictInfoHidingTGSubtaskSpec(DictInfoHidingTGSubtaskSpecBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(DictInfoHidingTGSubtaskSpec, self).__init__(
            partial_obs_keys,
            goal_keys,
            env,
            delta_t_ach,
    ...
    def goal_achievement_criterion(self, achieved_goal, desired_goal, parent_info):
        ...
        for key, threshold in self._goal_achievement_threshold.items():
            if (
                np.linalg.norm(
                    achieved_goal[key] - desired_goal[key], ord=self._norms[key]
                )
                >= threshold
            ):
                return False
...
class DictInfoHidingTolTGSubtaskSpec(DictInfoHidingTGSubtaskSpecBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(DictInfoHidingTolTGSubtaskSpec, self).__init__(
            partial_obs_keys,
            goal_keys,
            env,
            delta_t_ach,
        ...
        if max_goal_thresholds is None:
            max_goal_thresholds = {key: 1.0 for key in goal_keys}
        else:
            assert max_goal_thresholds.keys() == goal_keys
        self._goal_tol_space = DictSpace(
            {
                key: BoxSpace(low=[0.0], high=[value])
    ...
    def get_achieved_timed_goal_dict(self, achieved_goal, delta_t_ach, parent_info):
        """delta_t_ach assumed not to be mapped to [-1., 1.] yet"""
        atg = super(DictInfoHidingTolTGSubtaskSpec, self).get_achieved_timed_goal_dict(
            achieved_goal, delta_t_ach, parent_info
        )
