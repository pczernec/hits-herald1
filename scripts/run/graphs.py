...
from graph_rl.graphs.herald_graph import HeraldGraph

from scripts.run.interruption_policies.string_to_ip import get_ip_callable
...
def create_graph(env, graph_params, run_params, subtask_specs, level_algo_kwargs_list):
    ...
    graph = graph_func_dict[graph_params["algorithm"]](
        ...
        **graph_params["additional_kwargs"]
        if "additional_kwargs" in graph_params
        else {},
    )
...
def create_graph_hits(
    env, n_layers, n_steps, subtask_specs, level_algo_kwargs_list, **kwargs
):
...
def create_graph_herald(
    env, n_layers, n_steps, subtask_specs, level_algo_kwargs_list, **kwargs
):
    """Create and return Herald graph."""
    max_ep_len = env.max_episode_length if hasattr(env, 'max_episode_length') else env.spec.max_episode_steps

    if "child_failure_penalty" not in level_algo_kwargs_list[-1]:
        # Make the same child_failure_penalty as in HiTS
        testing_penalty_normalization = level_algo_kwargs_list[1].pop(
            "testing_penalty_normalization", 1
        )

        level_algo_kwargs_list[-1]["child_failure_penalty"] = (
            -max_ep_len / subtask_specs[0]._delta_t_max / testing_penalty_normalization
        )
    ...
    graph = HeraldGraph(
        ...
        HAC_kwargs=level_algo_kwargs_list[-1],
        HiTS_kwargs=level_algo_kwargs_list[:-1],
        HERALD_kwargs=kwargs["herald"],
        update_tsgs_rendering=env.update_timed_subgoals
        if hasattr(env, "update_subgoals")
        else None,
        node_kwargs=kwargs["node"]
    )
