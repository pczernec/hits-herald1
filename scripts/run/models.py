...
def get_mlp_models(level_params_list:dict, run_params:Optional[dict]=None):
    ...
    for level_params in level_params_list:
        ...
        if "goal_sampling_strategy" in algo_kwargs:
            algo_kwargs["goal_sampling_strategy"] = string_to_strategy(
                algo_kwargs["goal_sampling_strategy"]
            )
        # specify model
        algo_kwargs["model"] = get_mlp_model(
            **model_kwargs,
            ...
            squash_low=model_kwargs["squash_low"]
            if model_kwargs["squash_critics"]
            else None,
            squash_high=model_kwargs["squash_high"]
            if model_kwargs["squash_critics"]
            else None,
            device=device
        )
...
def get_mlp_model(
    ...
    squash_low=None,
    squash_high=None,
    actor_clip_threshold=None,
    critic_clip_threshold=None,
    device="cpu",
    force_negative=False,
):

    if flat_algo in ("SAC", "VariableDiscountSAC"):
        ModelClass = SACModel
    elif flat_algo == "DDPG":
