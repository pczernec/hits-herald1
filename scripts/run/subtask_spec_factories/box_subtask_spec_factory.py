...
class BoxSubtaskSpecFactory(SubtaskSpecFactory):
    ...
    def get_hits_subtask_specs(cls, env, n_layers, subtask_spec_params_list):
        ...
        if delta_t_max_list[-1] == -1:
            max_episdode_len = (
                env.max_episode_length if hasattr(env, 'max_episode_length') else env.spec.max_episode_steps
            )
            delta_t_max_list[-1] = max_episdode_len/subtask_spec_params_list[-1]["max_n_actions"]
        ...
        if ("type" in subtask_spec_params_list[-1] 
            ...
            highest_spec = EnvSPSubtaskSpec(
                max_n_actions_highest, env, cls.get_map_to_env_goal(env), constant_failure_return_highest,
